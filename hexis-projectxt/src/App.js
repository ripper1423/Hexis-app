...
export default function App(){
  ...
  const [exercises,setExercises]=useState(()=>{ const p=loadProfile(); return p&&WORKOUTS[p]?loadExercises(WORKOUTS[p].length):Array(5).fill(false); });
    ...
    {tab==="tips"&&(
      <div style={scr}>
        ...
        <div style={{padding:"16px 16px 0"}}>
          ...
          <div onClick={()=>{if(window.confirm('¿Resetear tu perfil y empezar de nuevo?')){clearAll();setProfile(null);setPlan(null);setWeightLog([]);setStreakData({current:0,best:0});setHabits([false,false,false,false]);setExercises(Array(5).fill(false));setWater(0);}}} style={{textAlign:"center",padding:"12px",cursor:"pointer",marginTop:8}}>
