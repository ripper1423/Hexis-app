...
import { PROFILES, WORKOUTS, MEALS, getTodayWorkout } from './data/profiles';
import {
...
// ── EXERCISE DB SCREEN ───────────────────────────────────────────
function PerfilScreen({profile,p,isPro,onUnlocked,onBack,onReset}){
  return(
...
export default function App(){
  ...
  if(screen==="perfil") return <PerfilScreen profile={profile} p={PROFILES[profile]} isPro={isPro} onUnlocked={()=>setIsPro(true)} onBack={()=>setScreen(null)} onReset={()=>{
    if(window.confirm('¿Reiniciar la aplicación desde el principio? Se borrará todo tu progreso guardado en este dispositivo.')){
      clearAll();setProfile(null);setPlan(null);setWeightLog([]);setStreakData({current:0,best:0});setHabits([false,false,false,false]);setExercises(Array(5).fill(false));setWater(0);setScreen(null);
    }
  }}/>;
    ...
    {tab==="entreno"&&(
      <div style={scr}>
        ...
        <div style={{padding:"16px 16px 0"}}>
          ...
          <SLabel text="Ejercicios de hoy" right="Toca para ver técnica y descanso"/>
          {w.map(({name,sets,reps,weight,unit,muscle,rpe,lastWeek,rest,how},i)=>(
            <div key={i} onClick={()=>setExpandEx(expandEx===i?null:i)} style={{background:"#0c0c0c",border:`1px solid ${expandEx===i?"rgba(200,170,80,0.3)":exercises[i]?"rgba(200,170,80,0.1)":"#1a1a1a"}`,borderRadius:12,padding:"14px 16px",marginBottom:8,cursor:"pointer",opacity:exercises[i]&&expandEx!==i?0.5:1,transition:"all 0.2s"}}>
              <div style={{display:"flex",alignItems:"center",gap:12}}>
                ...
                <div style={{flex:1}}>
                  ...
                  {lastWeek>0&&!exercises[i]&&(
                    <div style={{fontSize:11,color:G,marginTop:4,opacity:0.7}}>↑ Semana pasada: {lastWeek}{unit} · Sube 2.5kg si completaste todo</div>
                  )}
                </div>
                <div onClick={(e)=>{e.stopPropagation();const willBeDone=!exercises[i];const newE=exercises.map((v,j)=>j===i?!v:v);setExercises(newE);saveExercises(newE);if(willBeDone)logExerciseToCloud(userId,{name,weight,reps,sets});}} style={{width:26,height:26,borderRadius:"50%",border:`1px solid ${exercises[i]?p.color:"#222"}`,background:exercises[i]?p.color:"transparent",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,color:"#050505",flexShrink:0}}>{exercises[i]?"✓":""}</div>
              </div>
              {expandEx===i&&(
                <div style={{marginTop:12,paddingTop:12,borderTop:"1px solid #1a1a1a"}}>
                  ...
                  <div style={{display:"flex",gap:16}}>
                    ...
                    <div><div style={{fontSize:8,color:"#8a8a8a",textTransform:"uppercase",letterSpacing:1,marginBottom:2}}>Progresión</div><div style={{fontSize:12,color:"#aaa",fontWeight:600}}>+2.5kg si completas todo</div></div>
                  </div>
                </div>
              )}
            </div>
